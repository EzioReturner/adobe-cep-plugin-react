...
const SiderMenu: React.FC<{ siderBar?: React.ReactNode; siteLogo?: React.ReactNode }> = props => {
  ...
  function createIcon(icon: string | React.ReactNode) {
    ...
    if (icon.$$typeof) {
      // return icon;
      return <span className="LK-menuIcon">{cacheIcon}</span>;
    }
  ...
  function getMenuTitle(name: string = '', parentName?: string, icon?: React.ReactNode) {
    ...
    return (
      <span>
        {createIcon(icon)}
        <span className={!parentName ? 'LK-antd-subMenu-title' : ''} title={localName}>
          {localName}
        </span>
      </span>
    );
  ...
  function getSubMenuOrItem(menu: RouteChild, parentName?: string) {
    ...
    return (
      <Menu.Item key={menu.path} className="LK-antd-menuItem">
  ...
  function getMenuItem(menu: RouteChild, parentName?: string) {
    ...
    return (
      ...
      >
        {createIcon(icon)}
        <span
          className={parentName ? 'LK-antd-menuItem-title' : 'LK-antd-subMenu-title'}
          title={localName}
        >
          {localName}
        </span>
  ...
  const RAMenu = props.siderBar || (
    <Menu
      className={classNames(
        'LK-menu',
        isHorizontalNavigator && 'LK-menu-horizontal',
        isDarkTheme && 'LK-menu-darkTheme',
        collapsed && 'LK-menu-collapsed'
      )}
  ...
  const VerticalMenu = (
    <aside
      className={classNames(
        'LK-navigator',
        // fixSiderBar && 'LK-navigator-fixSiderBar',
        collapsed && 'LK-navigator-collapsed',
        isInlineLayout ? 'LK-inlineLayout-navigator' : 'LK-splitLayout-navigator',
        isDarkTheme && 'LK-navigator-darkTheme'
      )}
    >
