...
export function getAuthority(str?: string | string[]) {
  const authorityString = typeof str === 'undefined' ? localStorage.getItem('LK-authority') : str;
  let authority;
  try {
    // @ts-ignore
...
export function setAuthority(authority: string | string[]) {
  ...
  localStorage.setItem('LK-authority', JSON.stringify(proAuthority));
}

// 清除权限
export function clearAuthority() {
  localStorage.removeItem('LK-authority');
}
